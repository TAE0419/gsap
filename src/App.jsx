...
const LiquidGlass = () => {
  ...
  useEffect(() => {
    ...
    /* ================================================================
       Background Typography Texture (배경 타이포그래피 캔버스 텍스처)
       ================================================================ */
    ...
    const drawBackground = () => {
      ...
      // 그라데이션 배경 생성
      const grd = bgCtx.createLinearGradient(0, 0, w, 0);
      grd.addColorStop(0, '#fffecb');
      grd.addColorStop(0.25, '#3dc8ff');
      grd.addColorStop(0.5, '#0066ff');
      grd.addColorStop(0.75, '#3dc8ff');
      grd.addColorStop(1, '#fffecb');
      bgCtx.fillStyle = grd;
      ...
      // 메인 타이틀 렌더링
      bgCtx.fillStyle = '#fffecb';
      bgCtx.textAlign = 'center';
      bgCtx.textBaseline = 'middle';

      const titleSize = Math.round(Math.min(w * 0.25, h * 0.35));
      const titleGap = titleSize * 1;
      const subtitleGap = titleSize * 1.55;
      const groupCenterY = h * 0.5;
      const firstLineY = groupCenterY - titleGap * 0.5;
      bgCtx.font = `500 ${titleSize}px 'Space Grotesk', sans-serif`;
      bgCtx.fillText('LIQUID', w * 0.5, firstLineY);
    ...
    /* ================================================================
       Droplet Data & Buffers (물방울 데이터 관리)
       ================================================================ */
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uRes: { value: new THREE.Vector2(app.clientWidth, app.clientHeight) },
    ...
    /* ================================================================
       Event Listeners (마우스 및 화면 크기 조절 이벤트)
       ================================================================ */
    let aspect = app.clientWidth / app.clientHeight;
    ...
    const handleResize = () => {
      ...
      renderer.setSize(app.clientWidth, app.clientHeight);
      renderer.setPixelRatio(Math.min(2, window.devicePixelRatio || 1));
      aspect = app.clientWidth / app.clientHeight;
      mat.uniforms.uRes.value.set(app.clientWidth, app.clientHeight);
      drawBackground();
    };
    window.addEventListener('resize', handleResize);

    /* ================================================================
       Physics & Simulation (물리 연산 루프)
       ================================================================ */
    const DAMP = 0.993;
    const MOUSE_R = 0.18;
    const MOUSE_F = 0.004;
    ...
    return () => {
      ...
      renderer.dispose();
      if (app.contains(domElem)) {
        app.removeChild(domElem);
      }
    };
  }, []);
  ...
  return (
    <div className="liquid-glass-wrapper">
      ...
      <div className="info">drag to interact &middot; click to spawn</div>
    </div>
  );
};
